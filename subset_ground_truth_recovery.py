...
def run_partial_sp(elicitation_format, rule, domain):
    ...
    grouped_votes = bootstrap_sample_df.groupby('questions')

    for idx, row in sp_partial_ground_truth.iterrows():
        question = row['questions']
        sp_ranking = row['ranking']

        # Get vote rankings only for this subset
        vote_subset = grouped_votes.get_group(question)
        vote_ranking = votes_function(vote_subset['votes'], aggregation_type='vote', rule=rule)

        # Restrict both to common elements
        common_alts = set(sp_ranking) & set(vote_ranking)
        sp_ranking = [a for a in sp_ranking if a in common_alts]
        vote_ranking = [a for a in vote_ranking if a in common_alts]
        gt_ranking = sorted(vote_ranking)
        
        # Evaluation at the subset level
        tau_sp, _ = kendalltau(gt_ranking, sp_ranking)
        tau_vote, _ = kendalltau(gt_ranking, vote_ranking)
        rho_sp, _ = spearmanr(gt_ranking, sp_ranking)
        rho_vote, _ = spearmanr(gt_ranking, vote_ranking)
...
# Initialize a list to store performance records
performance_records = []

# Loop over each elicitation format
for elicitation_format in df['elicitation_format'].unique():
    print(f"\n🔍 Running Partial-SP for Elicitation Format: {elicitation_format}")

    # Skip formats that do not produce full rankings
    ...
    for domain in domains:
        ...
        # Loop over voting rules
        for voting_rule in voting_rules:
            print(f"🧪 Evaluating rule: {voting_rule}")
            mean_tau_sp, ci_tau_sp, mean_tau_vote, ci_tau_vote, mean_rho_sp, ci_rho_sp, mean_rho_vote, ci_rho_vote = run_partial_sp(elicitation_format, voting_rule, domain)
...
# Save full per-domain performance table
performance_df = pd.DataFrame(performance_records)
performance_df.to_csv("voting_rule_performance_comparison_subset_4_org.csv", index=False)

print("\n📊 Voting Rule Performance Table (Per-Domain):")
print(performance_df)

# Save average performance across all domains
avg_performance = performance_df.groupby(['Elicitation Format', 'Voting Rule']).mean(numeric_only=True).reset_index()
avg_performance.to_csv("voting_rule_average_performance_subset_4_org.csv", index=False)
